...
const SpecialTours: React.FC = () => {
  ...
  const handleTourClick = (index: number) => {
    // Chỉ card đầu tiên có link
    if (index === 0) {
      navigate('/tour-mua-hoa-anh-dao');
    }
  };
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {viTexts.specialTours.tours.map((tour: string, index: number) => (
            <div 
              ...
              className="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow cursor-pointer p-6 text-center"
            >
